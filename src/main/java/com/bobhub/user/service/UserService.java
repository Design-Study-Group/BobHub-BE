...
public class UserService {
  ...
  public List<ActivityDto> getUserActivity(PrincipalDetails principalDetails) {
    ...
    dummyActivities.add(
        new ActivityDto(1L, "party", "점심 같이 먹을 사람 구해요!", LocalDate.now().minusDays(1).toString()));
    dummyActivities.add(
        new ActivityDto(
            2L, "betting", "오늘 점심 내기에서 이겼습니다!", LocalDate.now().minusDays(2).toString()));
    dummyActivities.add(
        new ActivityDto(
            3L, "restaurant", "'최고의 국밥' 집에 리뷰를 남겼습니다.", LocalDate.now().minusDays(5).toString()));
    dummyActivities.add(
        new ActivityDto(4L, "ai", "AI 챗봇에게 메뉴를 추천받았습니다.", LocalDate.now().minusDays(7).toString()));
    dummyActivities.add(
        new ActivityDto(
            5L, "party", "저녁 치킨 파티원을 모집합니다.", LocalDate.now().minusDays(10).toString()));
